...
GIT_QUIET_FLAGS="-c advice.detachedHead=false"
GIT_COMMAND_QUIET_FLAGS="--quiet"
...
if [ ! -d "zm-build" ] ; then
  ...
  git ${GIT_QUIET_FLAGS} clone ${GIT_COMMAND_QUIET_FLAGS} "${ZMBUILD_REPO_URL}"
fi
...
for nZmbuildTag in $(cat ${ZMBUILD_TAGS}); do
  cd zm-build
  git ${GIT_QUIET_FLAGS} checkout ${GIT_COMMAND_QUIET_FLAGS} ${nZmbuildTag}
  cd ..
...
for nrepo in $(cat ${AGGREGATED_REPOS} | sort | uniq) ; do
  git ${GIT_QUIET_FLAGS} clone ${GIT_COMMAND_QUIET_FLAGS} $nrepo ${TAG_REPO_TMP_DIR}
  cd ${TAG_REPO_TMP_DIR}
